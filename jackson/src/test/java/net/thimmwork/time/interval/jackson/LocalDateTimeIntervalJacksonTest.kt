...
class LocalDateTimeIntervalJacksonTest {
    ...
    @Test
    fun `deserialize JSON to LocalDateTimeInterval`() {
        val actualInterval = objectMapper.readValue(
                "{" +
                        " \"start\": \"2018-01-01T00:00:00\"," +
                        " \"end\": \"2019-01-01T00:00:00\" " +
                        "}",
                LocalDateTimeInterval::class.java)

        val expected = parse("2018-01-01T00:00:00", "2019-01-01T00:00:00")
        assertEquals(expected, actualInterval)
    }

    @Test
    fun `serialize LocalDateTimeInterval to JSON`() {
        val instantInterval = parse("2018-01-01T00:00:00", "2019-01-01T00:00:00")
        val actualJson = objectMapper.writeValueAsString(instantInterval)

        val expected = "{\"start\":\"2018-01-01T00:00\",\"end\":\"2019-01-01T00:00\"}"
